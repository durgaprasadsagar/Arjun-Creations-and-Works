( () => {
    ...
    function s(t) {
        ...
        for (const [t,i] of this.formData) {
            ...
            if (n)
                if ("" === n.groups.array)
                    this.tree.set(n.groups.name, i);
                else {
                    const t = [...n.groups.array.matchAll(/\[([a-z][-a-z0-9_:]*|[0-9]*)\]/gi)].map(( ([t,e]) => e));
        ...
        if (!t.getAll(this.field).every((t => t instanceof File && this.accept?.some((e => /^\.[a-z0-9]+$/i.test(e) ? t.name.toLowerCase().endsWith(e.toLowerCase()) : (t => {
            ...
            if (s) {
                ...
                for (const [n,o] of ( () => {
                    ...
                    ("*" === i && o.startsWith(t + "/") || o === s[0]) && e.push(...n.split("|"))
            }
            return e
        }
        )(e).some((e => (e = "." + e.trim(),
    ...
    window.swv = {
        validators: e,
        validate: (t, s, o={}) => {
            const a = (t.rules ?? []).filter(( ({rule: t, ...s}) => "function" == typeof e[t] && ("function" != typeof e[t].matches || e[t].matches(s, o))));
            if (!a.length)
                return new Map;
            const r = new i(s)
              , p = a.reduce(( (t, s) => {
